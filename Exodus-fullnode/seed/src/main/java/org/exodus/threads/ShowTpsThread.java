...
import org.exodus.util.ExcelUtils;
import org.exodus.util.StringUtils;

/**
 * 统计及记录tps线程 Created by Clare on 2018/7/2 0002.
 */
public class ShowTpsThread extends Thread {
	private static final Logger logger = LoggerFactory.getLogger("ShowTpsThread.class");
	private Main node;
	...
	/**
	 * 打印信息： TPS、EPS、total transactions、total events Average transactions latency
...
//                    logger.info("eventData: " + eventData);
				...
				if (!StringUtils.isEmpty(firstTime) && !StringUtils.isEmpty(latestTime)) { // total costTime,
																							// firstTime不变
...
//            logger.info("++++++++++++++++++++++++++++ the eps of shard-" + i + ": " + shardAvgEps[i]);
		...
		logger.info("********************************************* {} *******************************************",
				index);
		logger.info("**************************** the average total cost time for all shards: {} sec",
				avgTotalTimesSeries[index]);
		logger.info("**************************** the average total transactions for all shards: {}",
				avgTotalTransactionsSeries[index]);
		logger.info("**************************** the average total events for all shards: {}",
				avgTotalEventsSeries[index]);
		logger.info("**************************** the average latency for all shards: {} sec",
				(avgTotalTimesSeries[index] / avgTotalEventsSeries[index]));
		logger.info("**************************** transactions per sec(shard tps plus): {}", tpsPlusSeries[index]);
		logger.info("**************************** transactions per sec(total transactions / total times): {}",
				tpsAvgSeries[index]);
	...
	private void writeResult(int fileCount) throws Exception {
		ExcelUtils excelUtils = new ExcelUtils(
				Config.DEFAULT_CONFIG_PATH + File.separator + "result-" + fileCount + ".xls");

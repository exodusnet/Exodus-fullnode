...
//    private final static String OTHER_PRICE_URL = "http://openapi.chainfin.online/openapi/v2/dcmarket/hq/exchangeRatios";
    private final static String OTHER_PRICE_URL = "http://openapi.inve.one/openapi/v2/dcmarket/hq/exchangeRatios";
     ...
     * @return nrg 价格
     */
    public static String getNrgPrice() {
        JSONObject o = new JSONObject();
        o.put("nrgPrice", Constant.DEFAULT_NRG_PRICE.toString());
    ...
    public static List<LocalFullNode> queryLocalfullnodeList(GeneralNode node) {
        List<LocalFullNode> localfullnodes = node.getLocalFullNodeList().values().stream()
    ...
    public static String registerRelayNode(String pubkey, String ip, int port, String data, GeneralNode node) throws Exception {
        if (node.openServiceState >= 1 && !node.getRatios().isEmpty()) {
            ...
            if (StringUtils.isEmpty(phone)) {
                logger.error("parameter phone is illegal. value: {}", phone);
                return ResponseUtils.paramIllegalResponse();
            }
            if (StringUtils.isEmpty(email) || !ValidateUtils.isEmail(email)) {
                logger.error("parameter email is illegal. value: {}", email);
                return ResponseUtils.paramIllegalResponse();
            }
            if (StringUtils.isEmpty(name) || name.length() > 10) {
                logger.error("parameter name is illegal. value: {}", name);
                return ResponseUtils.paramIllegalResponse();
            }
            if (StringUtils.isEmpty(addressesStr)) {
                logger.error("parameter addresses is illegal. value: {}", addressesStr);
                return ResponseUtils.paramIllegalResponse();
            }
            if ((feeRatio > 1.0 && feeRatio < 0.0)) {
                logger.error("parameter feeRatio is illegal. value: {}", feeRatio);
                return ResponseUtils.paramIllegalResponse();
            }
            ...
            logger.warn("openServiceState: {}, ratios: {}, msg: {}", node.openServiceState, node.getRatios(), msg);
            return ResponseUtils.response(203, msg);
        }
    }
    ...
    public static String queryRelayNodeList(String data, GeneralNode node) {
        if (node.openServiceState >= 1 && !node.getRatios().isEmpty()) {
            ...
            for (Iterator<Map.Entry<String, String>> iterator = node.getRatios().entrySet().iterator(); iterator.hasNext(); ) {
                ...
                logger.info("exchange ratio key: {}, value: {}", entry.getKey(), entry.getValue());
            }
            ...
            logger.info("relayNodeList : {}", JSONArray.toJSONString(list));
            //2019.04.19 更改兑换汇率
            String exchangeRatios = getExchangeRatios();
            logger.info("exchangeRatios : {}", exchangeRatios);
            if (StringUtils.isEmpty(exchangeRatios)) {
                return ResponseUtils.handleExceptionResponse();
            }
            for (int i = list.size() - 1; i >= 0; i--) {
                list.get(i).setExchangeRatios(JSONObject.parseObject(exchangeRatios, HashMap.class));
     ...
     * @return
     */
    private static String getExchangeRatios() {
        String exchangeRatios = null;
        try {
            String resultStr = HttpUtils.httpGet(OTHER_PRICE_URL + "?name=" + String.join(",", ChainTypes.NAMES.values()));
            logger.info("getExchangeRatios resultStr: {}", resultStr);
            if (StringUtils.isEmpty(resultStr)) {
                logger.error("getExchangeRatios response is null");
                return null;
            }
            JSONObject resultJson = JSONObject.parseObject(resultStr);
            if (0 != resultJson.getInteger("code")) {
                logger.error("getExchangeRatios response code is not 200");
                return null;
            }
            String jsonStr = resultJson.getString("json");
            JSONObject json = JSON.parseObject(jsonStr);
            exchangeRatios = json.getString("exchangeRatios");
            return exchangeRatios;
        } catch (Exception e) {
            logger.error("getExchangeRatios handle error: {}", e);
            return null;
        }
    }

    public static void main(String[] args) {
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW0"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW1"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW2"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW3"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW4"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW5"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW6"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW7"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW8"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RW9"));
        System.out.println(generateShardIdByPubKey(2, "A78IhF6zjQIGzuzKwrjG9HEISz7/oAoEhyr7AnBr3RWa"));

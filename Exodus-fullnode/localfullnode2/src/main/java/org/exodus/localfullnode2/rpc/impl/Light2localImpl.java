...
import org.exodus.localfullnode2.http.CommonApiService;
import org.exodus.localfullnode2.nodes.LocalFullNode1GeneralNode;
import org.exodus.localfullnode2.rpc.Light2local;
import org.exodus.localfullnode2.store.mysql.QueryTableSplit;
...
public class Light2localImpl implements Light2local {
	...
	public Light2localImpl(LocalFullNode1GeneralNode node) {
		this.node = node;
	}

	/**
	 * The rpc server receives the message from a light node. 1. validate message
	 * signature 2. for the transaction message, the rpc server put the message into
	 * the message queue. { "fromAddress": "OXV2C5ZQ7REPHM2SA3G4JS3T73ZNQOBD",
	 * "toAddress": "U2VRUTMT4ZFMQTZTFTHOSUHIZJNFHFTE", "amount": 1, "timestamp":
	 * 1536300281, "pubkey": "A5RA5jwayqjubpbtYFd87zaphu8erm5iNDBqiNOB6TB1", "fee":
	 * 124, "type": 1, "signature":
	 * "D9JG5+/aAgYRdPcn+y1eMompfvJjKUGW5zgfZo1M9vZ7MJ97yQN1R3mpM1TwKtQnPmywCIs4Xs13qFG7q9qqFQ=="
	 * }
	 * 
	 * @param message .
	 * @param current
	...
	/**
	 * 交易分库分表查询 查询一次无数据，再次查询下一张表(最多查询两次)
	 * 
	...
	public String getTransactionHistorySplit(String tableIndex, Long offset, String address, String type,
		...
		TransactionArray array = split.queryTransaction(new BigInteger(tableIndex), offset, address, type,

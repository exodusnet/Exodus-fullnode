...
//            logger.error("deleteEventsBeforeSnapshotPointEvent save error: {}", e);
	 ...
	 */
	public Map<Long, EventKeyPair> getPrevEventKeyPairsForEachNode(String dbId, EventBody eb,
			Map<Long, EventKeyPair> map, int nValue) {
	...
	public Map<Long, EventKeyPair> getFirstExistEventKeyPairsForEachNode(String dbId, int nValue) {
		...
		if (null != snapshotMessage) {
			SnapshotMessage sm = querySnapshotMessageByVersion(dbId, String
					.valueOf(snapshotMessage.getSnapVersion().intValue() - Config.DEFAULT_SNAPSHOT_CLEAR_GENERATION));
			if (null != sm) {
				Map<Long, EventKeyPair> map = new HashMap<>();

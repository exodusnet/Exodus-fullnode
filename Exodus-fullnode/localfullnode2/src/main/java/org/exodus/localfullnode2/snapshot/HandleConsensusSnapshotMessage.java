		...
		if (valid) {
			...
			try {
				validState = TxVerifyUtils.verifyMessageWithoutSign(JSONObject.parseObject(message),
						msgObject.getInteger("eShardId"), new RocksJavaUtil(dep.getDbId()), dep.getMultiple(),
						dep.getShardCount());
			} catch (Exception e) {
...
//                node.getTreeRootMap().get(snapshotMessage.getSnapVersion()) );
		...
		logger.info(">>>>>START<<<<<verifySnapshotMessage:\n snapshotMessage: {},\n treeRootMap: {}",
				JSON.toJSONString(snapshotMessage), JSON.toJSONString(dep.getTreeRootMap()));
	...
	private void handleRewardOfSnapshot(SnapshotMessage snapMsg, String eHash) throws InterruptedException {
		logger.info(">>>>>START<<<<<handleRewardOfSnapshot:\n snapshotMessage: {},\n eHash: {}",
				JSON.toJSONString(snapMsg), eHash);
		// 保存奖励记录
...
//                            snapMsg.getSnapVersion(), amount);
	...
	private void addRewardTx2SaveQueue(String mHash, String toAddress, BigInteger amount, BigInteger snapVersion)

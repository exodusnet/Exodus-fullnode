...
import org.exodus.localfullnode2.utilities.TxVerifyUtils;
import org.exodus.localfullnode2.utilities.TxVerifyUtils.Pair;
...
public class CommonApiService {
	...
	public synchronized static String sendMessage(String message, LocalFullNode1GeneralNode node) {
		...
		try {
			...
			// introduce balance checking mechanism when tranfering
			if (!TxVerifyUtils.hasAccountSufficientFunds(message, node.nodeParameters().dbId)) {
				logger.error("not enough fund in sender address");
...
//                    mlogger.info(node.nodeParameters().dbId + "|" + message);
	 ...
	 * @return 交易列表
	 */
	public synchronized static SystemAutoArray querySystemAuto(BigInteger tableIndex, long offset, String address,
			String type, LocalFullNode1GeneralNode node) {
		QueryTableSplit queryTableSplit = new QueryTableSplit();
		return queryTableSplit.querySystemAuto(tableIndex, offset, node.nodeParameters().dbId, address, type);
	}

	public synchronized static MsgArray querySystemAutoToMessageList(BigInteger tableIndex, long offset,
			LocalFullNode1GeneralNode node, String address) {
		QueryTableSplit queryTableSplit = new QueryTableSplit();
		SystemAutoArray systemAutoArray = queryTableSplit.querySystemAuto(tableIndex, offset,
				node.nodeParameters().dbId, address, "contract_fee_tx");
	...
	public synchronized static MsgArray querySystemAutoToMessageList(BigInteger tableIndex, long offset,
		...
		QueryTableSplit queryTableSplit = new QueryTableSplit();
		SystemAutoArray systemAutoArray = queryTableSplit.querySystemAuto(tableIndex, offset,
				node.nodeParameters().dbId, "contract_fee_tx");
		if (systemAutoArray == null || systemAutoArray.getList() == null || systemAutoArray.getList().size() == 0) {
			return null;
		}
		MsgArray msgArray = new MsgArray();

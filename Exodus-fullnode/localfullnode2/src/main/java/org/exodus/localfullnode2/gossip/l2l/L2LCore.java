package org.exodus.localfullnode2.gossip.l2l;
...
import org.exodus.localfullnode2.rpc.Event;
...
//	}
	...
	/**
	 * support old-fashion method which returns all higher events()
	 */
	public synchronized GossipObj gossipMyMaxSeqList4Consensus(String requesterPubkey, String requesterSig,
			String requesterSnapVersion, String requesterSnapHash, long[] requesterSeqs, BigInteger mySnapshotVersion,
			List<LocalFullNode> localFullnodes, IEventStore myStore, int shardId, String dbId) {
		return gossipMyMaxSeqList4Consensus(requesterPubkey, requesterSig, requesterSnapVersion, requesterSnapHash,
				requesterSeqs, mySnapshotVersion, localFullnodes, myStore, shardId, dbId, -1);
	}

	/**
	 * if {@code slot} is greater than or equal to 0,higher events in that specified
	 * slot are returned(if existed).
	 */
	public synchronized GossipObj gossipMyMaxSeqList4Consensus(String requesterPubkey, String requesterSig,
			String requesterSnapVersion, String requesterSnapHash, long[] requesterSeqs, BigInteger mySnapshotVersion,
			List<LocalFullNode> localFullnodes, IEventStore myStore, int shardId, String dbId, int slot) {
		GossipObj gossipObj = null;
		...
		if (currSnapshotVersion.equals(new BigInteger(requesterSnapVersion))
				|| currSnapshotVersion.subtract(BigInteger.ONE).equals(new BigInteger(requesterSnapVersion))
				|| currSnapshotVersion.add(BigInteger.ONE).equals(new BigInteger(requesterSnapVersion))) {
			List<Event> rpcEvents = null;

			if (slot < 0) {
				rpcEvents = getUnknownEvents(myStore, shardId, requesterSeqs).stream()
						.map(eventBody -> new Event(eventBody.getShardId(), eventBody.getCreatorId(),
								eventBody.getCreatorSeq(), eventBody.getOtherId(), eventBody.getOtherSeq(),
								eventBody.getTrans(), eventBody.getTimeCreated().getEpochSecond(),
								eventBody.getTimeCreated().getNano(), eventBody.getSignature(), eventBody.isFamous(),
				...
				rpcEvents = getUnknownEventsBySlot(myStore, shardId, requesterSeqs, slot).stream()
						.map(eventBody -> new Event(eventBody.getShardId(), eventBody.getCreatorId(),
								eventBody.getCreatorSeq(), eventBody.getOtherId(), eventBody.getOtherSeq(),
								eventBody.getTrans(), eventBody.getTimeCreated().getEpochSecond(),
								eventBody.getTimeCreated().getNano(), eventBody.getSignature(), eventBody.isFamous(),
	...
	/**
	 * improved {@code getUnknownEvents},which return events by slot.
	 */
	private ArrayList<EventBody> getUnknownEventsBySlot(IEventStore myStore, int shardId, long[] otherCounts,
			int slot) {
		...
		for (long j = otherCounts[slot] + 1L; j <= currMyCounts[slot]; ++j) {
			eventBody = myStore.getEventInMem(shardId, (long) slot, j);
			if (eventBody != null) {

...
public class SnapshotDependentImpl implements SnapshotDependent {
	...
	public List<LocalFullNode> getLocalFullNodes() {
		...
		for (int i = 0; i < 10; i++) {
			LocalFullNode localFullNode = new LocalFullNode.Builder().shard(String.valueOf(getShardId()))
					.index(String.valueOf(i)).address(Config.CREATION_ADDRESSES.get(i)).build();

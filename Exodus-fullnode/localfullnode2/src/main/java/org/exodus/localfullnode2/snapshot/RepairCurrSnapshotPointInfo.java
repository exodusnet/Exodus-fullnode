...
public class RepairCurrSnapshotPointInfo {
	...
	public void repairCurrSnapshotPointInfo(RepairCurrSnapshotPointInfoDependent dep, SnapshotDbService store)
...
//                                            temp.getConsEventCount(), evt.getConsEventCount());
									logger.error(
											">>>>>ERROR<<<<<repairCurrSnapshotPointInfo:\n consEventCount diff,\n "
													+ "calcu: {},db: {}",
											temp.getConsEventCount(), evt.getConsEventCount());
									System.exit(-1);
								}
...
//            logger.info(">>>>>END<<<<<repairCurrSnapshotPointInfo");
	...
	private void createSnapshotPoint(EventBody event) throws InterruptedException {
		if (dep.getTotalConsEventCount().mod(BigInteger.valueOf(Config.EVENT_NUM_PER_SNAPSHOT))
			...
			dep.putSnapshotPointMap(dep.getCurrSnapshotVersion(),
					new SnapshotPoint.Builder().eventBody(event).msgHashTreeRoot(dep.getMsgHashTreeRoot())
							.contributions((null != statistics && statistics.size() <= 0) ? null : statistics).build());
	...
	private SnapshotPoint calculateLatestSnapshotPoint() {
		...
		if (null != dep.getSnapshotPointMap()
			...
			lastSnapshotPoint = dep.getSnapshotPointMap().get(dep.getCurrSnapshotVersion().subtract(BigInteger.ONE));
		}
		logger.info(">>>>>RETURN<<<<<calculateLatestSnapshotPoint: lastSnapshotPoint: {}",
				JSON.toJSONString(lastSnapshotPoint));
...
//        logger.info(">>>>>START<<<<<calculateMsgHashTreeRoot:\n eventBody: {}", JSON.toJSONString(event));
			...
			for (byte[] msg : event.getTrans()) {
				...
				if (StringUtils.isEmpty(dep.getMsgHashTreeRoot())) {
					...
					dep.setMsgHashTreeRoot(
							DSA.encryptBASE64(Hash.hash(dep.getMsgHashTreeRoot(), msgObj.getString("signature"))));
				}
	...
	private void repairMsgHashTreeRoot() {
		...
		for (BigInteger i = startId.add(BigInteger.ONE); i.compareTo(dep.getConsMessageMaxId()) <= 0; i = i
				.add(BigInteger.ONE)) {

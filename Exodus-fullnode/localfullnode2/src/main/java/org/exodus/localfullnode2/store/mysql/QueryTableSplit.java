...
//    	logger.info("findTransaction()...");
	...
	public SystemAutoArray querySystemAuto(BigInteger tableIndex, Long offset, String dbId, String address,
			String type) {
		SystemAutoArray array = new SystemAutoArray();
		MysqlHelper h = null;
		try {
	...
	private JSONObject querySystemAuto(MysqlHelper h, BigInteger tableIndex, String dbId, String hash) {
		...
		try {
			JSONObject jsonObject = h.executeQuery(sql.toString(), rs -> {
				...
				try {
					if (rs.next()) {
						byte[] transactionByte = new RocksJavaUtil(dbId)
								.get(rs.getString("type") + rs.getBigDecimal("id"));

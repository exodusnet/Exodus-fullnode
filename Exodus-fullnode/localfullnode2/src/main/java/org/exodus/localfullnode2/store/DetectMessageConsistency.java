...
import org.exodus.localfullnode2.store.rocks.TransactionSplit;
...
//        return msgMap;
	...
	private static List<MsgPair> queryAllMessagesFromMysql(String dbName) {
		...
		for (BigInteger i = BigInteger.ZERO; i.compareTo(idx) <= 0; i = i.add(BigInteger.ONE)) {
			try {
				...
				if (list == null) {
					list = h.executeQuery(sql, (rs, index) -> new MsgPair(new BigInteger(rs.getString("id")),
							rs.getString("hash"), rs.getInt("isValid") == 1));
				} else {
					list.addAll(h.executeQuery(sql, (rs, index) -> new MsgPair(new BigInteger(rs.getString("id")),
							rs.getString("hash"), rs.getInt("isValid") == 1)));
				}
			} catch (Exception ex) {
	...
	public static void repairRepeatMessageInfo(String dbName) {
		...
		for (BigInteger i = BigInteger.ZERO; i.compareTo(idx) <= 0; i = i.add(BigInteger.ONE)) {
			try {
				String sql = "select hash from " + Config.MESSAGES + Config.SPLIT + idx.toString()
						+ " group by hash having count(*) >1";
				if (repList == null) {
					repList = h.executeQuery(sql, (rs, index) -> rs.getString("hash"));
				} else {
					repList.addAll(h.executeQuery(sql, (rs, index) -> rs.getString("hash")));
				}
			} catch (Exception ex) {
		...
		for (String hash : repList) {
			...
			try {
				String sql = "select id, hash, isValid, updateTime from " + Config.MESSAGES + Config.SPLIT
						+ idx.toString() + " where hash='" + hash + "' limit 1";
				...
				for (JSONObject o : list0) {
					...
					if (null != msgBytes) {
						...
						if (StringUtils.isEmpty(o1.getString("id")) || !o.getString("id").equals(o1.getString("id"))) {
			...
			if (null != msgBytes) {
				try {
					...
					if (null == o.getBoolean("isStable")) {
						logger.error("\nmessage-{} not consensus: \nhash = {}\nmsg: {}", pair.getId().toString(),
								pair.getHash(), new String(msgBytes));
					}

 ...
 */
package org.exodus.contract.ethplugin.crypto;

import org.exodus.contract.ethplugin.config.SystemProperties;
import org.exodus.contract.ethplugin.crypto.jce.SpongyCastleProvider;
...
public class HashUtil {
    ...
    public static byte[] sha3omit12(byte[] input) {
       ...
       // return copyOfRange(hash, 12, hash.length);
        return copyOfRange(hash, 0, hash.length);
    }
    ...
    public static byte[] calcNewAddr(byte[] addr, byte[] nonce) {
        ...
        byte[] sha3Addr = sha3omit12(RLP.encodeList(encSender, encNonce));
        byte[] rip160Addr = ripemd160(sha3Addr);

        return new Base32().encode(rip160Addr);
    }

    /**
     * The way to calculate new address inside ethereum for {@link org.exodus.contract.ethplugin.vm.OpCode#CREATE2}

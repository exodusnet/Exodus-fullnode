...
package org.exodus.contract.tuple;

import org.exodus.contract.tuple.valueintf.IValue0;
import org.exodus.contract.tuple.valueintf.IValue1;
import org.exodus.contract.tuple.valueintf.IValue2;
...
public final class Triplet<A, B, C> extends Tuple implements IValue0<A>, IValue1<B>, IValue2<C> {
	...
	public static <X> Triplet<X, X, X> fromArray(final X[] array) {
		...
		if (array.length != 3) {
			throw new IllegalArgumentException(
					"Array must have exactly 3 elements in order to create a Triplet. Size is " + array.length);
		}
	 ...
	 * have more (or less) elements than the tuple to be created.
	 * </p>
	 * 
	 * @param          <X> the iterable component type
	 * @param iterable the iterable to be converted to a tuple
	...
	private static <X> Triplet<X, X, X> fromIterable(final Iterable<X> iterable, int index, final boolean exactSize) {
		...
		if (iter.hasNext() && exactSize) {
			throw new IllegalArgumentException(
					"Iterable must have exactly 3 available elements in order to create a Triplet.");
		}

package org.exodus.contract.inve.vm.program;
...
import org.exodus.contract.ethplugin.core.AccountState;
...
import org.exodus.contract.ethplugin.db.ContractDetails;
import org.exodus.contract.ethplugin.util.ByteArraySet;
import org.exodus.contract.ethplugin.util.ByteUtil;
import org.exodus.contract.ethplugin.util.FastByteComparisons;
import org.exodus.contract.ethplugin.util.Utils;
import org.exodus.contract.ethplugin.vm.DataWord;
import org.exodus.contract.ethplugin.vm.MessageCall;
import org.exodus.contract.ethplugin.vm.OpCode;
import org.exodus.contract.ethplugin.vm.PrecompiledContracts.PrecompiledContract;
import org.exodus.contract.ethplugin.vm.program.InternalTransaction;
import org.exodus.contract.ethplugin.vm.program.ProgramPrecompile;
import org.exodus.contract.ethplugin.vm.program.ProgramResult;
import org.exodus.contract.ethplugin.vm.program.listener.CompositeProgramListener;
import org.exodus.contract.ethplugin.vm.program.listener.ProgramListenerAware;
import org.exodus.contract.ethplugin.vm.program.listener.ProgramStorageChangeListener;
import org.exodus.contract.inve.INVEConfig;
import org.exodus.contract.inve.vm.VM;
import org.exodus.contract.inve.vm.hook.VMHook;
import org.exodus.contract.inve.vm.program.invoke.INVEProgramInvoke;
import org.exodus.contract.inve.vm.program.invoke.INVEProgramInvokeFactory;
import org.exodus.contract.inve.vm.program.invoke.INVEProgramInvokeFactoryImpl;
import org.exodus.contract.inve.vm.trace.INVEProgramTrace;
import org.exodus.contract.inve.vm.trace.INVEProgramTraceListener;
...
import static org.exodus.contract.ethplugin.util.BIUtil.*;
import static org.exodus.contract.ethplugin.util.ByteUtil.EMPTY_BYTE_ARRAY;
import static org.exodus.contract.ethplugin.util.ByteUtil.toHexString;
...
public class INVEProgram {
    ...
    private INVEProgramInvoke invoke;
    private INVEProgramInvokeFactory programInvokeFactory = new INVEProgramInvokeFactoryImpl();

    private ProgramOutListener listener;
    private INVEProgramTraceListener traceListener;
    ...
    private INVEProgramTrace trace = new INVEProgramTrace();
    ...
    private final INVEConfig inveConfig;
    private final VMHook vmHook;

    public INVEProgram(byte[] ops, INVEProgramInvoke programInvoke) {
        this(ops, programInvoke, (Transaction) null);
    }

    public INVEProgram(byte[] ops, INVEProgramInvoke programInvoke, SystemProperties config) {
        this(ops, programInvoke, null, config, VMHook.EMPTY);
    }

    public INVEProgram(byte[] ops, INVEProgramInvoke programInvoke, Transaction transaction) {
        this(ops, programInvoke, transaction, SystemProperties.getDefault(), VMHook.EMPTY);
    }

    public INVEProgram(byte[] ops, INVEProgramInvoke programInvoke, Transaction transaction, SystemProperties config,
            VMHook vmHook) {
        this(null, ops, programInvoke, transaction, config, vmHook);
    }

    public INVEProgram(byte[] codeHash, byte[] ops, INVEProgramInvoke programInvoke, Transaction transaction, SystemProperties config, VMHook vmHook) {
        this.config = config;
        ...
        this.vmHook = vmHook;
        this.traceListener = new INVEProgramTraceListener(config.vmTrace());
        this.memory = setupProgramListener(new Memory());
        this.stack = setupProgramListener(new Stack());
        this.originalRepo = programInvoke.getRepository().clone();
        this.storage = setupProgramListener(new Storage(programInvoke));
        this.trace = new INVEProgramTrace(config, programInvoke);
    ...
    public INVEProgram withCommonConfig(CommonConfig commonConfig) {
    ...
    public void verifyStackSize(int stackSize) {
        if (stack.size() < stackSize) {
            throw INVEProgram.Exception.tooSmallStack(stackSize, stack.size());
        }
    }
    ...
    private void createContractImpl(DataWord value, byte[] programCode, byte[] newAddress) {
        ...
        //  actual gas subtract
        DataWord gasLimit = inveConfig.getCreateGas(getGas());
        spendGas(gasLimit.longValue(), "internal call");

        // [2] CREATE THE CONTRACT ADDRESS
        AccountState existingAddr = getStorage().getAccountState(newAddress);
        boolean contractAlreadyExists = existingAddr != null && existingAddr.isContractExist(inveConfig);
        ...
        INVEProgramInvoke programInvoke = programInvokeFactory.createProgramInvoke(
        ...
        if (contractAlreadyExists) {
            ...
            VM vm = new VM(vmHook);
            INVEProgram program = new INVEProgram(programCode, programInvoke, internalTx, config, vmHook).withCommonConfig(commonConfig);
            vm.play(program);
        ...
        if (afterSpend < 0) {
            if (!inveConfig.getConstants().createEmptyContractOnOOG()) {
                result.setException(INVEProgram.Exception.notEnoughSpendingGas("No gas to return just created contract",
                        storageCost, this));
            } else {
                track.saveCode(newAddress, EMPTY_BYTE_ARRAY);
            }
        } else if (getLength(code) > inveConfig.getConstants().getMAX_CONTRACT_SZIE()) {
            result.setException(INVEProgram.Exception.notEnoughSpendingGas("Contract size too large: " + getLength(result.getHReturn()),
                    storageCost, this));
    ...
    public void callToAddress(MessageCall msg) {
        ...
        if (isNotEmpty(programCode)) {
            ...
            VM vm = new VM(vmHook);
            INVEProgram program = new INVEProgram(getStorage().getCodeHash(codeAddress), programCode, programInvoke, internalTx, config, vmHook)
    ...
    public void spendGas(long gasValue, String cause) {
        ...
        if (getGasLong() < gasValue) {
            throw INVEProgram.Exception.notEnoughSpendingGas(cause, gasValue, this);
        }
    ...
    public DataWord getOriginalValue(DataWord key) {
        return originalRepo.getStorageValue(getOwnerAddress().getData(), key);
    }
    ...
    public INVEProgramTrace getTrace() {
        return trace;
    }
    ...
    public int verifyJumpDest(DataWord nextPC) {
        if (nextPC.bytesOccupied() > 4) {
            throw INVEProgram.Exception.badJumpDestination(-1);
        }
        int ret = nextPC.intValue();
        if (!getProgramPrecompile().hasJumpDest(ret)) {
            throw INVEProgram.Exception.badJumpDestination(ret);
        }
        return ret;
    }
    ...
    public static class Exception {
        ...
        public static OutOfGasException notEnoughSpendingGas(String cause, long gasValue, INVEProgram program) {

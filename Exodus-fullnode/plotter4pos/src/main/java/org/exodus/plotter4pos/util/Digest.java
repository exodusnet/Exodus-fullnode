...
public interface Digest {
	...
	/**
	 * Input some bytes, then finalize the current hash computation and return the
	 * hash value in a newly-allocated array. The object is resetted.
	 *
	 ...
	 * provided output buffer. The {@code len} parameter contains the maximum number
	 * of bytes that should be written; no more bytes than the natural hash function
	 * output length will be produced. If {@code len} is smaller than the natural
	 * hash output length, the hash output is truncated to its first {@code len}
	...
	/**
	 * Clone the current state. The returned object evolves independantly of this
	 * object.
	 *
	 * @return the clone
	 */
	 ...
	 * ("{@code B}") will be inferred from the key length, by selecting the smallest
	 * multiple of {@code n} which is no smaller than the key length. For instance,
	 * for the Fugue-xxx hash functions, this function returns -4: the virtual block
	 * length B is the HMAC key length, rounded up to the next multiple of 4.
	 * </p>
	 *

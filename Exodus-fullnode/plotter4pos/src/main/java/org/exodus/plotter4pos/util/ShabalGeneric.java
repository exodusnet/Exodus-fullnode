...
public class ShabalGeneric implements Digest {
	...
	/**
	 * Create the object. The output size must be a multiple of 32, between 32 and
	 * 512 (inclusive).
	 *
	...
	/**
	 * Create a new instance with the same parameters. This method is invoked from
	 * {@link #copy}.
	 *
	...
	private static final int decodeLEInt(byte[] data, int off) {
		return (data[off + 0] & 0xFF) | ((data[off + 1] & 0xFF) << 8) | ((data[off + 2] & 0xFF) << 16)
				| ((data[off + 3] & 0xFF) << 24);
	}

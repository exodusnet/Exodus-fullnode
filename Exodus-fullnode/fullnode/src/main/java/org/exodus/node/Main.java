...
public class Main extends GeneralNode {
	...
	public List<FullNode> getFullNodeList(String pubkey) {
		...
		try {
			String connInfo = "Register:default -h " + this.parameters.seedGossipAddress.pubIP + " -p "
					+ this.parameters.seedGossipAddress.rpcPort;
	...
	protected void loadConf(String[] args) {
		...
		implant.implantStaticConfig(false);
		implant.implantEnv();
		communicator = Util.initialize(implant.implantZerocConf());
		this.parameters = implant.implantParameters(false);

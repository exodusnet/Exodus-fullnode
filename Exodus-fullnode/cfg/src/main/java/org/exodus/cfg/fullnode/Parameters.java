package org.exodus.cfg.fullnode;
...
import org.exodus.cfg.core.StringUtils;
...
public class Parameters {
	...
	/**
	 * 环境： 开发测试环境还是线上环境 dev test online
	 */
	public String env;
	/**
	 * 默认文件前缀
...
//                logger.warn("{}: {}", keyValuePair[0], System.getProperty(keyValuePair[0]));
		...
		try {
			...
			this.keysFile = getConfigParam(communicator, paramMap, "keysFile", this.prefix + Config.DEFAULT_KEYS_FILE,
					true);
			this.walletFile = getConfigParam(communicator, paramMap, "walletFile",
					this.prefix + Config.DEFAULT_WALLET_FILE, true);
			this.shardSize = getConfigParam(communicator, paramMap, "shard.size", Config.DEFAULT_SHARD_SIZE, true);
			...
			this.whiteList = getConfigParam(communicator, paramMap, "whitelist", Config.DEFAULT_WHITE_LIST, false);
			this.blackList = getConfigParam(communicator, paramMap, "blackList", Config.DEFAULT_BLACK_LIST, false);
			...
			logger.info("test.whiteList : {}", JSONArray.toJSONString(whiteList));
		} catch (Exception e) {
			e.printStackTrace();
	...
	private String getConfigParamFromFile(Communicator communicator, String name) {
		return communicator.getProperties().getProperty(name.toLowerCase());
	}
}

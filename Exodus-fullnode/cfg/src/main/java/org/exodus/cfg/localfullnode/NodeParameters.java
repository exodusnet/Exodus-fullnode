	...
	public void init(Communicator communicator, String[] args) {
		...
		try {
			...
			this.selfGossipAddress.pubIP = getConfigParam(communicator, paramMap, "myself.pubIP",
					Config.DEFAULT_SEED_PUBIP, true);
			this.selfGossipAddress.gossipPort = Integer
					.parseInt(getConfigParam(communicator, paramMap, "myself.gossipPort", null, true));
			this.selfGossipAddress.rpcPort = Integer
					.parseInt(getConfigParam(communicator, paramMap, "myself.rpcPort", null, true));
			this.selfGossipAddress.httpPort = Integer
					.parseInt(getConfigParam(communicator, paramMap, "myself.httpPort", null, true));
			// 测试用的消息倍数
...
//                        getConfigParam(communicator, paramMap, "test.clearDb", "1", true) );
			...
			this.mnemonic = getConfigParam(communicator, paramMap, "mnemonic", "", false);
